...
### Added
- **TileSet support** — load and cut tile sheets into individual tiles using `cutTileSet()`.
- **CSV map loading** — load maps from CSV files using `load_room_csv()`.
- **Room transitions** — switch between maps using `trigger_room()`.
- **Support for multiple layers** in map rendering.

### Changed
- Updated `draw_layer()` to work with tile lists instead of atlas dictionaries.
- Documentation updated for version 0.3.
- README updated for version 0.3.

### Fixed
- Collisions now work correctly with CSV-loaded maps.
- Tile IDs in `blocked_tiles` now match the format used in maps (strings with single quotes).
- Example `test.py` updated to demonstrate all new features.
